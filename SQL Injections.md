SQL injections is the most common type of attack.
## Cheat Sheet
...
Consider an application that uses tracking cookies. Requests to the application include a cookie header like this:
...
Next, we send the following input (`SUBSTRING` function is called `SUBSTR` on some database) :
